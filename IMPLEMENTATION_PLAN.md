...
第十六批（#36，已合并）实现签名授权决定和本地防回滚状态原语（ADR-0018）：区分失联、到期和明确吊销，独立 Ed25519 验签、精确运行绑定、12h 签发上限与并发/重放/时钟负向测试。不新增公共授权 API 或迁移；中心事务签发/续期、连续 fence、持久化回写、command 与 AGT-005 仍待完成。
