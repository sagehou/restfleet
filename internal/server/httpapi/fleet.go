...
func (a *API) fleetProblem(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, control.ErrForbidden):
		a.problem(w, r, http.StatusForbidden, "ROLE_DENIED", "Request denied", "Administrator access is required.", nil)
	case errors.Is(err, domain.ErrRepositoryBusy):
		a.problem(w, r, http.StatusConflict, "REPOSITORY_BUSY", "Conflict", "A storage operation or repository lease is active.", nil)
	case errors.Is(err, domain.ErrRepositoryUnavailable):
		a.problem(w, r, http.StatusConflict, "REPOSITORY_UNAVAILABLE", "Conflict", "Only an uninitialized provisioning repository can be initialized.", nil)
	...
	case errors.Is(err, domain.ErrCredentialTestBusy):
		a.problem(w, r, http.StatusConflict, "CREDENTIAL_TEST_BUSY", "Conflict", "A storage operation is already active.", nil)
