...
UI MUST 明示 PROVISIONING 暂不可备份；format_version 缺省显示“尚未验证”，快照与容量显示尚未采集而非虚构 0。保存的凭据版本不表示已被 Agent 接受。失败请求显示错误，不伪装为空列表；创建冲突提示刷新列表确认已有结果。ADMIN 可对尚未初始化的 PROVISIONING 仓库提交初始化任务；UI MUST 从 last_initialize_operation_id 恢复轮询，模糊网络失败的手动重试 MUST 复用 key。成功显示中心验证时间与格式，仍明确等待 Gateway/Agent ACK；失败显示固定错误码，允许新任务重试。维护、轮换等未交付动作 MUST NOT 显示为可用。
