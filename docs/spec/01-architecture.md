...
- 不执行无法审计的维护操作；
- gateway 可在有效的中心预签发授权内独立继续接受备份流量；目标离线模型见 ADR-0017 与部署规范 09 §7.10，现有在线适配器尚未实现该能力。

独立 Gateway MUST 留在中心信任区域，通过受保护本地通道交付授权、短期运行材料与待回写记录，不新增公网管理接口、网络化内部微服务或必需消息服务。PostgreSQL 仍是权威；Gateway 本地记录只是待验证输入，不直接更新中心状态。控制面失联时撤销/禁用最多延迟 12h，Gateway 整体停止或丢失材料不承诺离线恢复。此限制不取消 AGT-005，续期边界与完整离线实验仍须独立验收。
